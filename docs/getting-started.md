...
| Antigravity CLI | The skills and agents are loaded and invoked through Antigravity |
...
Install Antigravity if you haven't already:
...
```bash
curl -fsSL https://raw.githubusercontent.com/mohit626689/geo-seo-claude/main/install.sh | bash
```

### macOS / Linux — manual

```bash
git clone https://github.com/mohit626689/geo-seo-claude.git
cd geo-seo-claude
./install.sh
```
...
curl -fsSL https://raw.githubusercontent.com/mohit626689/geo-seo-claude/main/install-win.sh | bash
...
- Copies the `geo` orchestrator skill to `~/.gemini/config/skills/geo/`
- Copies 13 sub-skills to `~/.gemini/config/skills/geo-*/`
- Copies 5 subagent definitions to `~/.gemini/config/agents/`
...
After installation, open Antigravity in any project directory and run:

```
/geo quick https://example.com
```

If the skill is wired up correctly, Antigravity will start a 60-second GEO visibility snapshot. If you see "unknown command" or nothing happens, restart Antigravity — it reads skills and agents at startup.

To confirm the files landed in the right place:

```bash
ls ~/.gemini/config/skills/geo/
ls ~/.gemini/config/skills/ | grep geo
ls ~/.gemini/config/agents/ | grep geo
```
...
- Symptom: installer warns `Antigravity CLI not found in PATH`
- Cause: `claude` is not installed or not on `PATH`
- Fix: `npm install -g @anthropic-ai/antigravity`; confirm with `claude --version`

**Skills not showing up in Antigravity**
- Symptom: `/geo quick` produces "unknown command" or no response
- Cause: Antigravity reads skills at startup; it won't see files added after launch
- Fix: fully quit and reopen Antigravity
...
- Fix: run manually from the cloned repo or from `~/.gemini/config/skills/geo/`:
  ```bash
...
This removes `~/.gemini/config/skills/geo/`, all `~/.gemini/config/skills/geo-*/` sub-skills, and all `~/.gemini/config/agents/geo-*.md` agent files. Python packages are not removed.
